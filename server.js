// ===============================================================================
// APEX TITAN v116.0 (SNIPER MASTER OVERLORD) - ULTIMATE ENGINE
// ===============================================================================
// MERGE SYNC: v115.0 (TRIANGLE) + v24.1 (SNIPER MASTER) + LIVENESS HEARTBEAT
// ===============================================================================
...
// --- CONFIGURATION (v24.1 SNIPER ARCHITECTURE) ---
const GLOBAL_CONFIG = {
    ...
    // STRATEGY SETTINGS (v24.1 Specs)
    WHALE_THRESHOLD: parseEther("15.0"), 
    MIN_LOG_ETH: parseEther("10.0"),      
    GAS_LIMIT: 1400000n,                 // v24.1 upping to 1.4M for stability
    MIN_NET_PROFIT: "0.01",              
    MIN_PROFIT_BUFFER: "0.005",          
    MARGIN_ETH: "0.012",                 // Leviathan floor
    PRIORITY_BRIBE: 15n,                 
...
async function fetchAIAssessment(ethPrice) {
    ...
    const userQuery = `ETH: $${ethPrice}. Suggest if Sniper Master strikes should be aggressive (BUY) or defensive (SELL).`;
    try {
...
╔════════════════════════════════════════════════════════╗
║   ⚡ APEX SNIPER MASTER v116.0 | ULTIMATE OVERLORD    ║
║   MODE: DUAL WHALE SNIPING + TRIANGLE SNIPER ACTIVE   ║
╚════════════════════════════════════════════════════════╝${TXT.reset}`);
...
async function initWorker(CHAIN) {
    ...
    async function safeConnect() {
        try {
            ...
            console.log(`${TXT.green}✅ CORE ${cluster.worker.id} [${ROLE}] SNIPER ACTIVE${TXT.reset}`);
            ...
            // v24.1 LIVENESS HEARTBEAT
            setInterval(async () => {
                try {
                    await wsProvider.getBlockNumber(); // Ping connection
                    if (priceFeed) {
                    ...
                    process.exit(1); // Force master to reboot core
                }
            }, 15000);
...
async function executeStrikeInternal(provider, wallet, strikeData, loanAmount, gasOracle, ethPrice, CHAIN, mode) {
    try {
        ...
        if (rawProfit > totalThreshold) {
            ...
            console.log(`\n${TXT.green}${TXT.bold}💎 ${mode} AUTHORIZED [${CHAIN.name}]${TXT.reset}`);
